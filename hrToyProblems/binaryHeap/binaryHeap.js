...
function BinaryHeap(sort) {
  ...
  this._isValidIndex = (index) => { return index >= 0 && index < this._heap.length }
  ...
  this._smallestChild = (indexOfParent) => {
    const children = [];
    let firstChild = indexOfParent * 2 + 1;
    let secondChild = indexOfParent * 2 + 2;
    if (this._isValidIndex(firstChild)) { children.push(firstChild); }
    if (this._isValidIndex(secondChild)) { children.push(secondChild); }
    if (children.length === 0) { return undefined }
    if (children.length === 1) { return children[0] }
    return this._heap[children[0]] < this._heap[children[1]] ? children[0] : children[1];
  };
...
BinaryHeap.prototype.insert = function(value) {
  ...
  while (insertIndex > 0 && this._compare(this._heap[insertIndex], this._heap[this._parent(insertIndex)])) {
    this._swap(insertIndex, this._parent(insertIndex));
    insertIndex = this._parent(insertIndex);
  }
}
...
BinaryHeap.prototype.removeRoot = function() {
  ...
  let index = 0;
  let smallestChild = this._smallestChild(index);
  while (smallestChild && this._compare(this._heap[smallestChild], this._heap[index])) {
    this._swap(smallestChild, index);
    index = smallestChild;
    smallestChild = this._smallestChild(index);
  }
  return root;
}
...
const heapSort = (array) => {
  const sorted = [];
  const heapSort = new BinaryHeap();
  array.forEach(value => { heapSort.insert(value) })
  let removedValue;
  while (true) {
    const removedValue = heapSort.removeRoot();
    console.log(removedValue);
    if (removedValue >= 0) { sorted.push(removedValue) } else { break; }
  }
  return sorted;
}
