...
var Range = function(start, end = start, step) {
...
Range.prototype.size = function() {
  const range = Math.abs(this._end - this._start) + 1;
  const size = Math.ceil(range / Math.abs(this._step));
  return size;
};

Range.prototype.each = function(callback) {
  for (let i = this._start; this._step > 0 ? i <= this._end : i >= this._end; i += this._step) {
    callback(i);
  }
};

Range.prototype.includes = function(val) {
  const smallest = Math.min(this._start, this._end);
  const largest = Math.max(this._start, this._end);
  const withinRange = val >= smallest && val <= largest;
  const start = 0;
  const end = this._end - this._start;
  const multipleOfStep = (val - this._start) % this._step === 0
  return withinRange && multipleOfStep;
};
