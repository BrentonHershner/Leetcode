 ...
 *
 * Complexity:
 *   What is the complexity of your algorithm in time and space? O(n*log(n))
 *   The merge step can be implemented using what is conceptually an insertion sort, and yet its time
 *   complexity is (spoiler alert!) much lower. Why is that? It's not checking every value against every other
 *
 *
 * Extra credit:
...
var mergeSort = function(array) {
  ...
  let lower = array.slice(0, midpoint);
  let upper = array.slice(midpoint);
  lower = mergeSort(lower);
  upper = mergeSort(upper);
  // console.log(`lower ${JSON.stringify(lower)} upper ${JSON.stringify(upper)}`)
...
// var input = [4, 7, 4, 3, 9, 1, 2];
// console.log(JSON.stringify(mergeSort(input)));
// var input = [4, 7, 4, -6, 9, 1, 2];
// console.log(JSON.stringify(mergeSort(input)));
// var input = ['dog', 'cat', 'boy', 'girl', 'bob'];
// console.log(JSON.stringify(mergeSort(input)));
